...
- [ ] 1. Pure validators, formatters, and capability guard
  ...
  - [ ] 1.1 Implement destination validation and E.164 normalization (`web/src/call/dialer-validation.ts`)
  ...
  - [ ] 1.3 Implement call-duration formatter (`web/src/call/duration-format.ts`)
  ...
  - [ ] 1.5 Implement DTMF digit validation (`web/src/call/dtmf-validation.ts`)
  ...
  - [ ] 1.7 Implement the capability guard (`web/src/call/capability-guard.ts`)
...
- [ ] 2. WebRtcCallClient (browser WebRTC session)
  ...
  - [ ] 2.1 Implement `WebRtcCallClient` core lifecycle (`web/src/call/webrtc-call-client.ts`)
  ...
  - [ ] 2.2 Implement mute, DTMF, teardown, and timers on `WebRtcCallClient`
  ...
  - [ ] 2.3 Implement the media-inactivity watchdog via `getStats()` polling
...
- [ ] 3. CallController state machine (single source of truth)
  ...
  - [ ] 3.1 Implement the `CallController` store, phases, and ws subscriptions (`web/src/call/call-controller.ts`)
  ...
  - [ ] 3.2 Implement outbound `placeCall` and inbound `answer` with mic-first ordering
  ...
  - [ ] 3.3 Implement Ringing progress state and the Outbound_No_Answer_Timeout timer
  ...
  - [ ] 3.4 Implement inbound presentation, decline, hangup, duration, and DTMF routing
  ...
  - [ ] 3.5 Implement the single-call guard and terminal-event teardown (idempotent, callId-keyed)
  ...
  - [ ] 3.6 Implement `call_cancelled`/answered_elsewhere handling and reconnect reconciliation
...
- [ ] 4. Checkpoint - Core call engine (client + controller) tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Device lifecycle (browser device as a call target)
  ...
  - [ ] 5.1 Implement `deviceLifecycle` reuse, unload beacon, and logout (`web/src/call/device-lifecycle.ts`)
  ...
  - [ ] 5.2 Implement reconnect reconciliation, registration timeout, and retry
...
- [ ] 7. Call-control icons and token-based call styles
  Design-system integration groundwork the surfaces depend on: SVG icons and token-first CSS (no hardcoded hex).

  - [ ] 7.1 Add call-control icons to `web/src/components/icons.tsx`
  ...
  - [ ] 7.2 Add token-based call surface styles to `web/src/styles/main.css`
...
- [ ] 8. Call UI surfaces
  ...
  - [ ] 8.1 Implement the Dialer popover/sheet (`web/src/components/dialer.tsx`)
  ...
  - [ ] 8.2 Implement the IncomingCallSurface modal (`web/src/components/incoming-call-surface.tsx`)
  ...
  - [ ] 8.3 Implement the InCallSurface compact overlay with expandable keypad (`web/src/components/call-banner.tsx`)
...
- [ ] 9. Wire call surfaces and the nav "dial" affordance into the app
  Single `CallController` per tab, rendered by the App shell; a new nav affordance launches the Dialer.

  - [ ] 9.1 Instantiate `CallController` and render surfaces from `main.tsx` (`web/src/main.tsx`)
  ...
  - [ ] 9.2 Add the "dial" affordance and call-back entry points (`web/src/components/nav.tsx`, `web/src/components/call-history.tsx`)
...
- [ ] 10. Server-side stale Web_Device reaper
  The one new server component plus two small read accessors. Backs Property 6.

  - [ ] 10.1 Add broadcaster and orchestrator read accessors
  ...
  - [ ] 10.2 Implement the `WebDeviceReaper` sweep (`src/services/web-device-reaper.ts`)
  ...
  - [ ] 10.3 Wire the reaper at server bootstrap (`src/server.ts`)
...
- [ ] 11. Checkpoint - Client, surfaces, and reaper integrated
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Integration verification and final wiring
  ...
  - [ ] 12.3 Final end-to-end wiring and regression pass
...
- [ ] 13. Final checkpoint - All components integrated and tests pass
  - Ensure all tests pass, ask the user if questions arise.
