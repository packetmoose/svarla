...
- **UI surfaces** — `Dialer` (compose/place), `IncomingCallSurface` (ringing answer/decline), `InCallSurface` (connecting/ringing/connected status, mute, DTMF keypad, duration, volume, hang up). All three are **distinct surfaces rendered by `main.tsx` from `CallController` state** (see [UI layout and presentation](#ui-layout-and-presentation)): the `Dialer` is **launched as a popover/modal from a new "dial" affordance in `nav.tsx`** (not a route), the `IncomingCallSurface` is a centered modal + scrim, and the `InCallSurface` is a **compact anchored overlay at rest that expands to reveal the DTMF keypad + volume via a keypad toggle** (the keypad is toggled open, not always visible). All three are token-styled and icon-based per the design system: `InCallSurface` supersedes the old `📞`/ad hoc banner, presenting a token-based surface with SVG icons; the surfaces are theme-aware via tokens (see [UI Design System Integration](#ui-design-system-integration)).
...
- **DTMF keypad presentation.** The 12-key DTMF keypad (`0`–`9`, `*`, `#`; the twelve controls of Requirement 7.1) is **not always visible**: it is revealed by **expanding the compact `InCallSurface` overlay via the keypad toggle** (see [UI layout and presentation](#ui-layout-and-presentation)). When expanded, each key drives `sendDtmf` and remains **keyboard-operable** and **touch-target sized** (`--min-touch-target` 48px on touch/small screens), satisfying Requirement 7.1 (twelve controls), the accessibility requirements (Requirement 18), and the design-system touch-target rule while keeping the resting surface compact.
...
`CallController.store` is a `createStore<CallControllerState>` instance (`web/src/state.ts`). A small `useCallState()` hook subscribes components to it. `main.tsx` instantiates one `CallController` for the app and renders the three **distinct** surfaces — `Dialer`, `IncomingCallSurface`, and `InCallSurface` (the upgraded `call-banner.tsx`) — from its state (see [UI layout and presentation](#ui-layout-and-presentation)). The `Dialer` is **not a route**: it is opened as a popover/modal from a new "dial" affordance in `nav.tsx` (and from "call back" entry points), so `main.tsx` renders it as an overlay gated by UI state rather than by the router. The `InCallSurface` renders as a **compact anchored overlay** that expands in place to reveal the DTMF keypad + volume when the user activates its keypad toggle (the keypad is toggled, not always visible); at most one such overlay exists per tab, consistent with the single-call invariant. The `Dialer` fetches originating numbers from the number management API (`number-routes` / `multi-provider-number-routes`) to populate the "from" selector (Requirements 2.1, 2.9) and offers "call back" from `call-history.tsx` (Requirement 17.2).
...
This subsection pins down the concrete **placement and presentation** of each call surface. The three surfaces are **distinct** and are rendered from the App shell (`web/src/main.tsx`) driven by `CallController` state — the Dialer popover, the `IncomingCallSurface` modal, and the `InCallSurface` overlay never share a container. Consistent with the single-call invariant (Property 1), **at most one `InCallSurface` overlay exists per tab**. All three honor the responsive breakpoints already referenced by the design system: touch/small-screen behavior applies at `max-width: 640px` or `pointer: coarse` (where `--min-touch-target` 48px is in effect), and the desktop vs. compact nav layout switches at the `768px` breakpoint used by `nav.tsx`.

- **Dialer — nav-launched popover/modal (desktop) / full sheet (mobile).** The Dialer is **not** a standalone route or page. It opens as a **popover/modal launched from a new "call/dial" affordance in the existing navigation** (`web/src/components/nav.tsx`): on desktop (≥768px) the nav renders as the sidebar and the Dialer presents as a **modal/popover anchored to the nav**; below 768px the nav is the top bar + hamburger and the Dialer opens from the same affordance. On mobile (≤640px / `pointer: coarse`) the Dialer presents as a **full-width sheet / full-screen** surface for touch ergonomics. The Dialer is also reachable via **"call back"** from `call-history.tsx` and, where relevant, from conversations (Requirement 17.2); those entry points open the same Dialer surface pre-filled with the destination. The nav gains a new **"dial"** affordance built with the `icons.tsx` convention (the new `phoneIcon` via `iconSvg(...)`, inheriting `currentColor`), sitting alongside the existing `homeIcon`/`chatIcon`/`callIcon`/`settingsIcon` items and carrying an `aria-label` for the icon-only control (Requirement 18). The Dialer panel uses `--md-surface` on a `--md-scrim` backdrop with `--md-radius-lg` and `--md-elevation-4` when presented modally, matching the other overlays.
- **InCallSurface — compact anchored overlay at rest, expandable keypad.** The `InCallSurface` (the upgraded `call-banner.tsx`) is a **compact overlay** at rest: a small **anchored banner/pill** (e.g. a bottom or top corner on desktop) showing the caller/number, the call status (Connecting / Ringing / Connected), the running call duration, and controls for **mute**, **hang up**, a **keypad toggle**, and a **volume** affordance. It uses `--md-elevation-3` at rest. When the user activates the **keypad toggle**, the surface **expands within the same overlay** to reveal the **DTMF dial pad** (the 12-key 3×4 grid) and the volume control; the keypad is **toggled open, not always visible**, so the resting surface stays compact. The expanded keypad still follows the design tokens (buttons at `--md-radius-button`, spacing on the 8px scale) and honors the touch-target rule: DTMF keys and the answer/decline/hang-up controls use `--min-touch-target` (48px) on touch/small screens. On mobile (≤640px / `pointer: coarse`) the `InCallSurface` presents **full-width / full-screen** rather than a corner pill.
- **IncomingCallSurface — centered modal + scrim, full sheet on mobile.** The `IncomingCallSurface` presentation is unchanged from what is already documented: a **centered modal/dialog** over a dimmed backdrop (`--md-scrim`), using `--md-radius-lg` and `--md-elevation-4`. On mobile (≤640px / `pointer: coarse`) it presents as a **full-width sheet**. The Answer and Decline buttons honor the touch-target sizing (`--min-touch-target` on touch/small screens) and use the color roles above (`--md-success`/`.btn-success-outline` for Answer, `--md-error`/`.btn-danger` for Decline).
...
11. **Compact expandable in-call overlay + nav-launched Dialer popover vs. full-screen in-call modal + dedicated Dialer route.** The in-call surface could have been a full-screen in-call modal on desktop, and the Dialer could have been a dedicated route/page. **Decision:** on desktop, present the `InCallSurface` as a **compact anchored overlay** that expands in place to reveal the DTMF keypad + volume (keypad toggled, not always visible), and launch the `Dialer` as a **popover/modal from a new "dial" affordance in `nav.tsx`** rather than as a route (see [UI layout and presentation](#ui-layout-and-presentation)). Rationale: this keeps the call controls **unobtrusive on desktop while staying reachable**, and matches the existing **nav-centric layout** (the Dialer sits with the other nav destinations instead of hijacking the main content area). Trade-off: on mobile (≤640px / `pointer: coarse`) both surfaces still go **full-width/full-screen** for touch ergonomics, so the presentation is responsive rather than uniform across form factors.
...
| UI layout and presentation (Dialer popover, InCallSurface compact expandable overlay, IncomingCallSurface modal; nav "dial" affordance; responsive 640/768 breakpoints) | 2 (Dialer nav-launched popover / mobile sheet), 3 (Incoming centered modal + scrim / mobile sheet), 6/7/8 (in-call controls incl. toggled DTMF keypad + volume), 18 (accessibility / touch targets), 19 (visual consistency) |
